...
```{.callout-tip}
**Consejo:** No intentes inventar todo desde cero. Usa una plantilla como punto de partida y adáptala a tus necesidades. Es como cocinar: puedes seguir una receta y luego agregar tus propios ingredientes.
```
...
```{.callout-note}
**Nota:** El modelo lineal funciona bien cuando tienes una idea clara de lo que quieres comunicar y el documento no es muy extenso. Si el documento crece, considerar un modelo más flexible.
```
...
```{.callout-tip}
**Consejo:** En Quarto, puedes usar la función `include:` o archivos parciales (`_intro.qmd`, `_metodos.qmd`, etc.) para crear secciones reutilizables. Así, un mismo método puede incluirse en varios informes sin copiar y pegar.
```
...
```{.callout-warning}
**Advertencia:** No confundas "iterativo" con "procrastinar". Cada iteración debe tener un objetivo claro: mejorar la claridad, corregir datos, ajustar el tono, etc. Define cuántas iteraciones necesitas antes de empezar.
```
...
```{.callout-note}
**Nota:** No necesitas ser programador para usar Git. Hoy en día existen interfaces visuales (como GitHub Desktop o VS Code) que hacen que el control de versiones sea accesible para cualquier persona.
```
...
```{.callout-tip}
**Consejo:** Si tu audiencia accede al documento desde dispositivos móviles, HTML es la mejor opción. El formato se adapta automáticamente al tamaño de la pantalla.
```
...
```{.callout-note}
**Nota:** Si envías tu documento a una revista académica o a un cliente formal, es muy probable que requieran el formato PDF.
```
...
```{.callout-warning}
**Advertencia:** No llenes las diapositivas de texto. Una presentación es un apoyo visual, no un documento para leer. Usa frases cortas, imágenes y gráficos.
```
...
```{.callout-tip}
**Consejo final:** No tienes que elegir un solo modelo. Puedes combinarlos. Por ejemplo: escribe de forma modular, revisa de forma iterativa y genera el resultado en HTML y PDF. La flexibilidad es tu mayor aliado.
```
...
```{.callout-note}
**Recuerda:** El mejor modelo es el que se adapta a tu necesidad, no al revés. Experimenta con diferentes combinaciones hasta encontrar la que te resulte más cómoda.
```
...
- Carroll, S. (2023). *Quarto: Authoring Books, Articles, Websites, Presentations, and More*. O'Reilly Media.
- RStudio Team. (2024). *Quarto Documentation*. Disponible en: https://quarto.org/docs/
- Git SCM. (2024). *Pro Git*. Disponible en: https://git-scm.com/book
